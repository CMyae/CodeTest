 ...
 */
class HttpLoggingInterceptor constructor(private val logger: HttpLoggingInterceptor.Logger = HttpLoggingInterceptor.Logger.DEFAULT) :
    Interceptor {
  ...
  override fun intercept(chain: Interceptor.Chain): Response {
    ...
    if (logHeaders) {
      ...
      if (!logBody || !HttpHeaders.hasBody(response)) {
        ...
        if (contentLength != 0L) {
          logger.log("")
          logger.log(buffer?.clone()?.readString(charset).orEmpty())
        }
